...
class TextToSpeech: NSObject, ObservableObject, AVAudioPlayerDelegate {
  private let ttsAPIUrl = "https://texttospeech.googleapis.com/v1beta1/text:synthesize"
  ...
  func speak(text: String, voiceType: VoiceType = .waveNetFemale, completion: @escaping () -> Void) {
    ...
    DispatchQueue.global(qos: .background).async {
      ...
      let headers = ["X-Goog-Api-Key": self.APIKey, "Content-Type": "application/json; charset=utf-8"]
